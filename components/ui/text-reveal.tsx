  ...
  return (
    ...
    >
      <div className="sticky top-0 mx-auto flex h-[60vh] max-w-4xl items-center px-[1rem] py-[3rem]">
        <p className="flex flex-wrap p-5 font-space text-2xl font-bold text-ink/20 md:p-8 md:text-3xl lg:p-10 lg:text-4xl xl:text-5xl">
          {words.map((word, i) => {
            const start = i * s
            const end = start + d
            return (
...
const Word: FC<WordProps> = ({ children, progress, range }) => {
  ...
  return (
    <span className="relative mx-1 lg:mx-2.5">
      ...
      <m.span data-testid="text-reveal-word" style={{ opacity }} className="text-ink">
        {children}
      </m.span>
    </span>
  )
